...
type
  TScaling = (scXPos, scYPos, scWidth, scHeight, scXCentred, scYCentred, scText);
  TScalings = set of TScaling;
  TScaleEvent = procedure(Sender: TObject; CurTextSize: Integer;
     var NewLeft, NewTop, NewWidth, NewHeight, NewTextSize: Integer) of object;
  ...
  TActiveFont = class(TFont)
  private
    fOnChange: TNotifyEvent;
    fChangeHandler: TNotifyEvent;
    procedure ExecChange(Sender: TObject);
  public
    constructor Create(ChangeHandler: TNotifyEvent);
  published
  ...
  private
    fControlsAdjusted: Boolean;
    procedure WMSizing(var Message: TMessage); message WM_SIZING;
    procedure WMNCHitTest(var Message: TMessage); message WM_NCHITTEST;
  protected
    procedure DoAdjustForSizing(const Value: Boolean); virtual;
  public
    procedure AdjustForSizing(const Value: Boolean); virtual;
    property ControlsAdjusted: Boolean read fControlsAdjusted;
  end;
...
constructor TActiveFont.Create(ChangeHandler: TNotifyEvent);
begin
  inherited Create;
  fChangeHandler := ChangeHandler;
  inherited OnChange := ExecChange;
end;

procedure TActiveFont.ExecChange(Sender: TObject);
begin
  fChangeHandler(Self);
...
procedure TScaledForm.AdjustForSizing(const Value: Boolean);
begin
  if Value = fControlsAdjusted then Exit;
  DoAdjustForSizing(Value);
end;

procedure TScaledForm.DoAdjustForSizing(const Value: Boolean);
begin
  fControlsAdjusted := Value;
end;

procedure TScaledForm.WMNCHitTest(var Message: TMessage);
begin
  inherited;
  if not(Message.Result=HTCLIENT) then
    DoAdjustForSizing(False);
end;

procedure TScaledForm.WMSizing(var Message: TMessage);
begin
  if not fControlsAdjusted then
    DoAdjustForSizing(True);
  inherited;
end;
